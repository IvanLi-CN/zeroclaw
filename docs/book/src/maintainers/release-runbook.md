...
tag and enable `publish_prebuilt`. This recovery path verifies the published
Linux archives against `SHA256SUMS`, publishes the prebuilt and generated
variant tags, and never recreates or moves the existing tag or GitHub Release.
Do not dispatch the full stable release workflow again for an existing tag.

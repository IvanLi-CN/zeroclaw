...
tool-telegram-sticker-description = Send a Telegram sticker selected by an exact emoji from the configured shared sticker sets. This only sends to the current Telegram conversation and never accepts raw Telegram file IDs. Use it at most three times in one turn. Use send_via for text before a sticker; for a sticker-only response, return NO_REPLY[INFO].
tool-telegram-sticker-param-emoji = Exact emoji selector from configured Telegram sticker metadata.
tool-telegram-sticker-error-action-blocked = Action blocked: { $error }
tool-telegram-sticker-error-raw-file-id = Telegram stickers must be selected by emoji; raw file_id values are not accepted.
tool-telegram-sticker-error-missing-emoji = Missing sticker emoji selector.
tool-telegram-sticker-error-empty-emoji = Sticker emoji selector must not be empty.
tool-telegram-sticker-error-no-turn = Telegram sticker delivery is only available while handling a Telegram channel turn.
tool-telegram-sticker-error-alias-not-configured = Telegram alias '{ $alias }' is not configured.
tool-telegram-sticker-error-missing-credentials = Telegram sticker delivery requires a configured bot token and API base URL.
tool-telegram-sticker-error-no-sets = No Telegram sticker sets are configured.
tool-telegram-sticker-error-transport = Telegram { $method } transport failure.
tool-telegram-sticker-error-response-read = Telegram { $method } response read failure.
tool-telegram-sticker-error-http = Telegram { $method } HTTP failure ({ $status }).
tool-telegram-sticker-error-response-decode = Telegram { $method } response decode failure.
tool-telegram-sticker-error-api = Telegram { $method } API failure.
tool-telegram-sticker-error-missing-metadata = Telegram getStickerSet response lacks sticker metadata.
tool-telegram-sticker-error-no-match = No configured Telegram sticker matches emoji '{ $emoji }'.
tool-telegram-sticker-error-quota = This turn already sent { $limit } stickers.
tool-telegram-sticker-error-empty-target = Telegram sticker delivery target is empty.
tool-send-via-description = Control where and how this turn's reply is delivered, or send an extra message to another channel. When a user requests a reply format or destination, call this tool at the start of the response. Without `body`, use `target` and/or `modality` to set the main reply route; at least one is required. With `body` and `target`, send separate content to another channel while the main reply remains on the originating channel. During an active Telegram turn, omit `target` with `body` to send text to the current chat before a sticker. A target is required with body outside an active Telegram turn. Target must be a channel alias such as `telegram.default` or a peer group that includes the active agent. Modality defaults to the peer group's output modality.
tool-send-via-param-target = Channel alias such as telegram.default or a peer group name. Required when body is present except in an active Telegram turn; otherwise omit to use the originating channel.
tool-send-via-param-modality = Delivery modality. Omit to inherit the peer group's output modality.
tool-send-via-param-body = Message content for an immediate send. Omit to set the routing instruction for the current reply instead.
tool-send-via-error-body-target-required = A target is required when body is present outside an active Telegram turn.
